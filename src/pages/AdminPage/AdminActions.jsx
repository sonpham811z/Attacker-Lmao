...
useEffect(() => {
  ...
  const handleSetMispricing = e => {
    e.preventDefault();
    setMispricingStatus('Chỉ thực hiện phạt sau khi admin xác nhận báo cáo!');
  };
          ...
          <DialogActions>
            ...
            {selectedReport && (
              <Button
                ...
                onClick={async () => {
                  setMispricingStatus('Đang gửi giao dịch...');
                  await handleResolveMispricing(selectedReport.validator);
                }}
                sx={{ ml: 2 }}
              >
                {reportResolved ? 'Đã xử lý' : 'Xác nhận phạt mispricing'}
              </Button>
            )}
          </DialogActions>
        </Dialog>
